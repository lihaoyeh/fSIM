...
%% Load fluorescent image stack with background subtraction

% filedir = ['D:\Li-Hao\cSIM\data\data_035\scmos_image2\'];
...
% % 
% nstart = [550,610];
% nstart_large = [108,60]+nstart;
% 
% bg = 300; % background noise
% I_image = zeros(Ns,Ms,Nimg);
% for m = 1:Nimg    
%     for j = 1:1
%         fn = [filedir,imglist(m).name];
%         I = double(imread(fn))-bg;
%         I = max(0,imresize(I,[1296,1536]));
% 
%         I_image(:,:,m) = I(nstart_large(1):nstart_large(1)+Ns-1,nstart_large(2):nstart_large(2)+Ms-1);
%     end
% end
% 
% save('registered_fluorescent_data','-v7.3');
load('registered_fluorescent_data');
...
for i = 1:Nimg
    temp = max(0,I_image(:,:,i));
    I_image_up(:,:,i) = max(0,real(iF(padarray(F(temp),[(N-Ncrop)/2,(M-Mcrop)/2]))));
end
...
%         T_incoherent = T_incoherent - grad_OTF/max(abs(I_multi_f(:)))^2/2;
